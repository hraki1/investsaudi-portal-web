Globe = function(p) {
	...
	p.load = function() {
		var geocodes = tsunami.load.json("/globe/geocodes.json");
		var mapImage = tsunami.load.image("/globe/images/earth_black.png");
		var pinImage = tsunami.load.image("/globe/images/pin.png");
		//var glowImage = tsunami.load.image("/globe/images/glow.png");
		var borders = tsunami.load.json("/globe/borders.json");
		var maps = tsunami.load.json("/globe/maps.json");
	...
	p.loadComplete = function(assets) {
		// Clean up existing renderer/canvas if this is a re-initialization
		if (this.renderer) {
			try {
				if (this.renderer.domElement && this.renderer.domElement.parentNode === this) {
					this.removeChild(this.renderer.domElement);
				}
				this.renderer.dispose();
			} catch (err) {
				console.warn("Globe renderer cleanup failed:", err);
			}
		}

		// Optional: cleanup secondary renderer if it was used
		if (this.renderer2) {
			try {
				if (this.renderer2.domElement && this.renderer2.domElement.parentNode === this) {
					this.removeChild(this.renderer2.domElement);
				}
				this.renderer2.dispose();
			} catch (err) {
				console.warn("Globe secondary renderer cleanup failed:", err);
			}
		}
